...
class RoleController extends Controller
{
    ...
    {
        $request->validate([
            'name' => 'requiered|unique:roles,name',
    ...
    public function update(Request $request, string $id)
    {
        $registro = Role::findOrFail($id);
        $request->validate([
            'name' => 'requiered|unique:roles,name'.$registro->id,
            'permissions' => 'required|array',
        ]);
        $registro = Role::create(['name' => $request->name]);
        $registro->syncPermissions($request->permissions);
    ...
    public function destroy(string $id)
    {
         ...
         $registro->delete();
          ...
          return redirect()->route('roles.index')->with('mensaje', 'Rol'.$registro->name,'Eliminado satisfactoriamente');
